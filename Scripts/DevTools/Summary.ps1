function Show-Summary {
    ...
    foreach ($category in $categoryStats.Keys | Sort-Object) {
        ...
        Write-Log ("{0,-20} {1,3}/{2,-3} ({3,5}%) [✓:{4} ⚠:{5} ✗:{6} Ⓜ:{7}]" -f 
            $category, 
    ...
    foreach ($category in $categoryStats.Keys | Sort-Object) {
        ...
        foreach ($toolName in ($categoryTools | Sort-Object)) {
            ...
            $status = switch ($InstallationResults[$toolName]) {
                ...
                default { "✓ Installed via $($InstallationResults[$toolName])" }
            }
            Write-Log "  $displayName".PadRight(30) + ": $status" -Level INFO
        }
    }
