function Install-ViaWinGet {
    ...
    if ($PSCmdlet.ShouldProcess($ToolName, "Install via WinGet")) {
        try {
            ...
            switch ($exitCode) {
                ...
                -1978335189 {
                    Write-Log "⚠ WinGet: Package may already be installed or requires elevation." -Level WARN
                    return $false
                }
                -1978335212 {
                ...
                default {
                    Write-Log "❌ WinGet installation failed (exit code: $exitCode)." -Level WARN
                    if ($result) { Write-Log "WinGet output: $result" -Level DEBUG }
                    return $false
                }
            }
        } catch {
    ...
    Write-Log "Skipped WinGet installation for $ToolName." -Level SKIP
    return $false
}
